...
2. 单镜时长 2.5–6 秒；叙事结构**按 Brief 内容二选一**：
   - **若 Brief 有 subject（画面主体）且 product.usp 为空或缺失** → 按**内容叙事**：
     建立(1) → 发展(1-2) → 高潮(1) → 收尾(1)。**不要**出现产品展示/卖点演示/CTA 这类营销环节，
     也不要虚构商品或品牌。本分支的 narration 与 on_screen_text 不得出现卖点/CTA/品牌名，copy 中若带 cta 也一律忽略。
   - **否则**（商品/营销 Brief）→ 按**营销结构**：
     hook(1) → 场景/痛点(1-2) → 产品展示(1-2) → 卖点演示(1-2) → CTA(1)
   - 镜头数较少时（≤2）取**所选分支**序列的前 N 个环节即可；镜头数为 3 时同样按所选分支压缩环节，但必须保留顺序并凑满总时长。
...
4. on_screen_text ≤ 14 字，必须是**观众真的会看到的字幕**（不是镜头名）。
   内容叙事类若无字幕需求，可给空串（该字段仍须存在，写 ""）；**营销结构每一镜都必须给字幕且至少 2 字**。
